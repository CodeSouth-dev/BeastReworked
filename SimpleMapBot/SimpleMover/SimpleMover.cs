...
{
    /// <summary>
    /// Custom player mover with optimized pathfinding and movement skill support
    /// </summary>
    public class SimpleMover : IPlayerMover
    {
        public static readonly ILog Log = Logger.GetLoggerInstanceForType();

        #region Implementation of IAuthored
        public string Name => "SimpleMover";
        public string Description => "A player mover with movement skill support";
        ...
        {
            get { return SimpleMoverSettings.Instance; }
        }
        ...
        public bool MoveTowards(Vector2i position, params dynamic[] user)
        {
            ...
            if (IsTargetPortal(position) && !SimpleMoverSettings.Instance.AllowPortalMovement)
            {
            ...
            if (_cmd == null ||
                ...
                (_sw.IsRunning && _sw.ElapsedMilliseconds > SimpleMoverSettings.Instance.PathRefreshRate) ||
                _cmd.Path.Count <= 2 ||
            ...
            if (canUseMoveSkill &&
                myPosition.Distance(point) > SimpleMoverSettings.Instance.SingleUseDistance &&
                (!_skillBlacklistStopwatch.IsRunning || _skillBlacklistStopwatch.ElapsedMilliseconds > 250))
            {
                ...
                if (skillMovePoint != Vector2i.Zero)
                {
                    ...
                    if (_casted)
                    {
                        if (SimpleMoverSettings.Instance.DebugLogging)
                        {
        ...
        private bool BasicMove(Vector2i myPosition, Vector2i point)
        {
                ...
                LokiPoe.Me.CurrentAction.Skill.InternalId.Equals("Move"))
            {
                ...
                if (myPosition.Distance(point) <= SimpleMoverSettings.Instance.SingleUseDistance)
                {
                    LokiPoe.ProcessHookManager.ClearAllKeyStates();
        ...
        private void CheckStuck(Vector2i currentPosition)
        {
            ...
            if (distanceMoved < SimpleMoverSettings.Instance.StuckDistance && _lastPosition != Vector2i.Zero)
            {
                _stuckCount++;
                ...
                if (_stuckCount > SimpleMoverSettings.Instance.StuckThreshold)
                {
            ...
            {
                if (_stuckCount > 0 && SimpleMoverSettings.Instance.DebugLogging)
                {
